        ...
        if not self.__read_data_ok:
            if self.binary_mode:
                if not isinstance(self.read_data, bytes):
                    raise TypeError(
                        f"{self.filename} opened in binary mode, expected read_data to be "
                        f"bytes, not {type(self.read_data).__name__}"
                    )
            else:
                if not isinstance(self.read_data, str):
                    raise TypeError(
                        f"{self.filename} opened in non-binary mode, expected read_data to "
                        f"be str, not {type(self.read_data).__name__}"
                    )

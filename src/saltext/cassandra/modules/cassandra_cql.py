    ...
    if not property_name:
        ...
        if not loaded_property:
            if set_default:
                ...
                raise CommandExecutionError(f"ERROR: Cassandra {config_option} cannot be empty.")
...
def cql_query_with_prepare(
    ...
    **kwargs,
    ...
    if results and not asynchronous:
        for result in results:
            values = {}
            for key, value in result.items():
                # Salt won't return dictionaries with odd types like uuid.UUID
...
def list_column_families(
    ...
    query = {
        "2": f"select columnfamily_name from system.schema_columnfamilies {where_clause};",
...
def keyspace_exists(
    ...
    query = {
        "2": (
            f"select keyspace_name from system.schema_keyspaces where keyspace_name = '{keyspace}';"
        ),
        "3": (
...
def create_keyspace(
    ...
    if not existing_keyspace:
        ...
        query = f"create keyspace {keyspace} with replication = {replication_map} and durable_writes = true;"
        try:
            cql_query(
                query,
                contact_points=contact_points,
...
def create_user(
    ...
    query = f"create user if not exists {username} with password '{password}' {superuser_cql};"
    log.debug(
